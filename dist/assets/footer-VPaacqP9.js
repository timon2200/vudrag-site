import{s as f}from"./state-DFY6wBk_.js";import"./config-DYBXrzFk.js";import{_ as E}from"./playcanvas-D5Kv4vzz.js";function h(e){console.log(`🧭 Navigating to: ${e}`),e==="hero"?(f.targetScrollProgress=0,f.isScrolling=!0):e==="category-hub"?(f.targetScrollProgress=1.2,f.isScrolling=!0,setTimeout(()=>{const t=document.getElementById("category-hub");t&&t.scrollIntoView({behavior:"smooth"})},100)):e==="artist"?(f.targetScrollProgress=1.4,f.isScrolling=!0,setTimeout(()=>{const t=document.getElementById("artist-section");t&&t.scrollIntoView({behavior:"smooth"})},100)):e==="works"?h("hero"):e==="contact"?window.location.href="/contact.html":console.warn(`Unknown navigation target: ${e}`)}const A=Object.freeze(Object.defineProperty({__proto__:null,navigateTo:h},Symbol.toStringTag,{value:"Module"})),S="/api";let i=null,_=!1;const u={links:[{label:"Gallery",target:"hero"},{label:"Collections",target:"category-hub"},{label:"Artist",target:"artist"},{label:"Inquire",target:"contact"},{label:"Collectors Club",href:"/login.html"}],brand:"Nikola Vudrag",tagline:"From Atom to Atlas"};async function C(){try{const e=new AbortController,t=setTimeout(()=>e.abort(),2e3),n=await fetch(`${S}/site-content`,{signal:e.signal});if(clearTimeout(t),!n.ok)throw new Error("CMS unavailable");const r=await n.json();return r.footer?{links:r.footer.navLinks&&r.footer.navLinks.length>0?r.footer.navLinks.map(o=>o.href.startsWith("#")?{label:o.label,target:o.href.substring(1)}:{label:o.label,href:o.href}):u.links,brand:r.footer.brand||u.brand,tagline:r.footer.tagline||u.tagline}:u}catch{return console.warn("⚠️ CMS unavailable, using fallback menu content"),u}}async function M(){if(document.querySelector(".menu-overlay"))return;const e=await C();i=document.createElement("div"),i.className="menu-overlay";const t=e.links.map(s=>s.target?`<a href="#" class="menu-link" data-target="${s.target}">${s.label}</a>`:`<a href="${s.href}" class="menu-link">${s.label}</a>`).join(`
            `);i.innerHTML=`
    ...
    `,document.body.appendChild(i),i.querySelector(".menu-close").addEventListener("click",b),i.querySelectorAll(".menu-link").forEach(s=>{s.addEventListener("click",$)}),window.addEventListener("toggle-menu",T),console.log("🍔 Menu overlay created (CMS-driven)")}function I(){i||M(),_=!0,i.classList.add("visible"),document.body.style.overflow="hidden"}function b(){i&&(_=!1,i.classList.remove("visible"),document.body.style.overflow="")}function T(){_?b():I()}function $(e){const t=e.target.getAttribute("data-target");t&&(e.preventDefault(),b(),h(t))}const g={ROOT_MARGIN:"-50px",THRESHOLD:.1,STAGGER_DELAY:100};let v=null;function j(){if(!("IntersectionObserver"in window)){console.warn("⚠️ IntersectionObserver not supported, revealing all"),B();return}const e=document.getElementById("content-area");v=new IntersectionObserver(x,{root:e||null,rootMargin:g.ROOT_MARGIN,threshold:g.THRESHOLD}),q(),console.log("✨ Scroll reveal system initialized")}function x(e){e.forEach(t=>{if(t.isIntersecting){const n=t.target;n.classList.add("is-revealed"),n.classList.contains("reveal-stagger")&&O(n),v.unobserve(n)}})}function O(e){const t=e.children;Array.from(t).forEach((n,r)=>{setTimeout(()=>{n.classList.add("is-revealed")},r*g.STAGGER_DELAY)})}function q(){const e=document.querySelectorAll("[data-reveal]"),t=document.querySelectorAll(".reveal-stagger");e.forEach(n=>{v.observe(n)}),t.forEach(n=>{v.observe(n)}),console.log(`📍 Observing ${e.length} reveal elements`)}function p(e){v&&e&&v.observe(e)}function B(){document.querySelectorAll("[data-reveal]").forEach(t=>{t.classList.add("is-revealed")})}const R="/api",c={brand:"VUDRAG",tagline:"From Atom to Atlas",description:"The same fire that engraves a coin's 1.2mm relief forges a 12-meter monument. At every scale, entropy yields to intent — chaos transformed into form.",email:"office@vudrag.com",location:"Varaždin • Zagreb • Dubai",navLinks:[{label:"Collections",href:"#category-hub"},{label:"Artist",href:"#artist-section"},{label:"Inquire",href:"/contact.html"},{label:"Collectors Club",href:"/login.html"}],socialLinks:[{name:"Instagram",icon:"instagram",url:"https://www.instagram.com/vudrag_art/"},{name:"Facebook",icon:"facebook",url:"https://web.facebook.com/nikola.vudrag.77"},{name:"Interview",icon:"article",url:"https://www.contemporaryartissue.com/a-conversation-with-nikola-vudrag/"}]};async function D(){try{const e=new AbortController,t=setTimeout(()=>e.abort(),2e3),n=await fetch(`${R}/site-content`,{signal:e.signal});if(clearTimeout(t),!n.ok)throw new Error("CMS unavailable");const r=await n.json();return r.footer?{brand:r.footer.brand||c.brand,tagline:r.footer.tagline||c.tagline,description:r.footer.description||c.description,email:r.footer.email||c.email,location:r.footer.location||c.location,navLinks:r.footer.navLinks&&r.footer.navLinks.length>0?r.footer.navLinks:c.navLinks,socialLinks:r.footer.socialLinks&&r.footer.socialLinks.length>0?r.footer.socialLinks:c.socialLinks}:c}catch{return console.warn("⚠️ CMS unavailable or timed out, using fallback footer content"),c}}async function Y(){const e=document.getElementById("content-area");if(!e||document.getElementById("main-footer"))return;const t=await D(),n=document.createElement("footer");n.id="main-footer",n.className="site-footer",n.innerHTML=`
